...
import { supabase, signUp, signIn, signOut, getSession, saveCompany, getCompany, saveAssessment, getAssessments } from './lib/supabase.js'
import { SECTORS, PRODUCT_CATEGORIES, LC_THRESHOLD } from './lib/sectors.js'
import { computeScore, getRecommendations, fmt, pct } from './lib/scoring.js'
...
function Feedback() {
  ...
  const handleSend = () => { if (!msg.trim()) return; setSent(true); setMsg(''); setTimeout(() => { setOpen(false); setSent(false) }, 2000) }
  return (
    <>
      ...
      {open && <div className="fade-in" style={{ position: 'fixed', bottom: 80, left: 24, width: 300, background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 16, padding: 20, zIndex: 50, boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
        ...
        {sent ? <div style={{ color: T.accent, fontSize: 14, fontWeight: 600, padding: '20px 0', textAlign: 'center' }}>Thank you for your feedback!</div> : <>
          ...
          <button onClick={handleSend} style={{ ...btnP, width: '100%', padding: '8px 16px', fontSize: 13 }}>Send</button>
        </>}
      </div>}
    </>
  )
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 1000, system: ctx(), messages: [...msgs.filter((_, i) => i > 0), um].map(m => ({ role: m.role, content: m.content })) }),
        signal: controller.signal
      })
      clearTimeout(timeout)
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const data = await res.json()
      const text = data.content?.map(b => b.text || '').join('') || 'I could not process that. Please try rephrasing.'
